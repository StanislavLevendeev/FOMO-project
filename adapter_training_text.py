...
def parse_args():
    parser = argparse.ArgumentParser(description="Train a CLIP-to-DINO text adapter.")
    parser.add_argument(
        "--config",
        default="config.yaml",
...
def contrastive_loss(img_features, txt_features, logit_scale):
    ...
    # Since image features are not passing through the adapter, we must ensure they are normalized
    img_features = F.normalize(img_features, p=2, dim=-1)
    # Text features are already normalized by the adapter, but it doesn't hurt to normalize again
...
def evaluate(model, dataloader, device):
    ...
    with torch.inference_mode():
        for img, txt in dataloader:
            ...
            projected_txt = model(txt)
            loss, norm_loss, accuracy = contrastive_loss(
                img, projected_txt, model.logit_scale
            )
...
def main():
    ...
    print(f"Building text adapter with architecture: {arch_name} ({arch_config['type']})")
    
    # We swap input_dim and output_dim since we are adapting from text (dim 512) to image (dim 384)
    ...
    for epoch in range(epochs):
        ...
        for img, txt in pbar:
            ...
            optimizer.zero_grad()
            # Pass text embeddings through the adapter
            projected_txt = model(txt)
            
            # Compute loss using the dynamic model temperature scale
            loss, norm_loss, accuracy = contrastive_loss(img, projected_txt, model.logit_scale)
            loss.backward()
    ...
    print(f"\nSaved last text adapter checkpoint to {last_path}")
    if best_val_accuracy is not None:
        print(f"Saved best text adapter checkpoint to {best_path}")
