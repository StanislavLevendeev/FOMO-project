...
from adapter_training_text import ProjectionAdapter, DeepResidualAdapter, ResidualAdapter, load_config
import os
...
def get_adapter(config_path="config.yaml", device="cpu"):
    ...
    # Notice input and output dims are swapped for text adapter!
    ...
    if models_dir.exists():
        ...
        for d in models_dir.iterdir():
            # Specifically search for text_adapter runs
            search_name = f"text_adapter-arch-{arch_name}".replace("_", "")
            if d.is_dir() and search_name in d.name.replace("_", ""):
    ...
    if not found_model_path:
        root_path = Path(f"text_adapter_{arch_name}.pt")
        if root_path.exists():
    ...
    if not found_model_path:
        ...
        print(f"You must first train the text '{arch_name}' adapter before evaluating it.")
        exit(1)
...
def evaluate_imagenet(adapter, device, batch_size):
    ...
    with torch.inference_mode():
        for i in tqdm(range(0, len(class_features), batch_size), desc="Projecting ImageNet Text"):
            batch = class_features[i:i+batch_size]
            projected = adapter(batch)
            mapped_text_features.append(projected)
            
    mapped_text_features = torch.cat(mapped_text_features, dim=0)
    
    acc = topk_accuracy_from_features(
        image_features=raw_image_features,
        class_features=mapped_text_features,
        labels=labels,
        topk=(1, 5),
...
def evaluate_flickr30k(adapter, device, batch_size):
    ...
    mapped_text_features = []
    with torch.inference_mode():
        for i in tqdm(range(0, len(text_features), batch_size), desc="Projecting Flickr30k Text"):
            batch = text_features[i:i+batch_size]
            projected = adapter(batch)
            mapped_text_features.append(projected)
    ...
    retrieval_metrics = flickr30k_retrieval_at_1(
        image_features=raw_image_features,
        text_features=mapped_text_features,
        image_ids=image_ids,
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate Text Adapter on ImageNet and Flickr30k")
    ...
    print(f"\nEvaluating Text Adapter: {arch_name}")
    ...
    print("\n" + "="*80)
    print(f"{'Text Adapter Name':<30} | {'Flickr30k I2T@1':<15} | {'Flickr30k T2I@1':<15} | {'ImageNet Top-1':<15} | {'ImageNet Top-5':<15}")
    print("-" * 80)
